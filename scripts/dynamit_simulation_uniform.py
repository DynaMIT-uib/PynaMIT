...
# Avoid setting wind at low latitudes
...
# Use the average conductances
#hall_aurora = np.mean(hall_aurora) * np.ones_like(hall_aurora)
#pedersen_aurora = np.mean(pedersen_aurora) * np.ones_like(pedersen_aurora)
...
dynamics.evolve_to_time(10)

jr = a.get_upward_current(mlat = mlat, mlt = mlt) * 1e-6
jr[np.abs(jr_lat) < 50] = 0 # filter low latitude jr
jr[(jr_lat >= 50) & (jr_lat < 60)] = np.mean(jr[(jr_lat >= 50) & (jr_lat < 60)]) * np.ones_like(jr[(jr_lat >= 50) & (jr_lat < 60)])
jr[(jr_lat >= 60) & (jr_lat < 70)] = np.mean(jr[(jr_lat >= 60) & (jr_lat < 70)]) * np.ones_like(jr[(jr_lat >= 60) & (jr_lat < 70)])
jr[(jr_lat >= 70) & (jr_lat < 80)] = np.mean(jr[(jr_lat >= 70) & (jr_lat < 80)]) * np.ones_like(jr[(jr_lat >= 70) & (jr_lat < 80)])

jr[jr_lat <= -50] = np.mean(jr[jr_lat <= -50]) * np.ones_like(jr[jr_lat <= -50])
print('norm', np.linalg.norm(jr))
dynamics.set_jr(jr, lat = jr_lat, lon = jr_lon)

dynamics.evolve_to_time(30)
