...
with tempfile.TemporaryDirectory() as directory:
    simulation = Simulation(
        ...
        main_field_epoch=decimal_year(event_time),
...
apex_lat, apex_lon = apex.geo2apex(geo_lat, geo_lon, 110.0)

dipole_mlt = Dipole(decimal_year(event_time)).mlon2mlt(apex_lon, event_time)

amps_mlt = pyamps.mlon_to_mlt(apex_lon, event_time, decimal_year(event_time))
...
request = ExternalInputRequest.from_model_coordinates(
    ...
    model_epoch=decimal_year(event_time),
)
hall, pedersen, _, _ = get_conductance_inputs(event_time, None, None, None, request=request)

amps = pyamps.AMPS(300.0, 0.0, -4.0, 20.0, 100.0, minlat=50.0)

jr = np.asarray(amps.get_upward_current(mlat=apex_lat, mlt=amps_mlt)) * 1e-6
