...
def test_cs_basis_state_end_to_end():
    ...
    assert state.geometry.input_adapter is not None, (
        "Hybrid input_adapter should be created w/ SHBasis/CSBasis mismatch"
    )
    ...
    # Verify bundled m_imp feedback-system construction.
    problem = state.m_imp_feedback_system.problem
    reduced_system = state.get_m_imp_reduced_system()
    assert reduced_system.n_reduced < reduced_system.n_total
    assert problem.solution_size == reduced_system.n_reduced
    ...
    gauge_row = np.asarray(state.constraints.get_m_imp_gauge_row(cs_basis.size), dtype=float)
    if gauge_row.size > 0:
        gauge_violation = np.linalg.norm(gauge_row @ m_imp.reshape(-1, 1))
        assert gauge_violation < 1e-10 * max(np.linalg.norm(m_imp), 1.0)
    ...
    if rel_residual >= 1.0:
        print(
            f"WARNING: High residual {rel_residual}. Check CSBasis.laplacian scaling or regularization."
        )
    else:
...
def test_cs_basis_m_imp_problem_respects_gauge_setting():
    """CS ``m_imp`` problem should switch between full and reduced coordinates."""
    N = 18
    cs_basis = CSBasis(N)
    sh_basis_dummy = SHBasis(Nmax=10, Mmax=8, mean_free=True)
    mainfield = Mainfield(MainfieldKind.DIPOLE)

    state_with_gauge = State(
        basis=sh_basis_dummy,
        mainfield=mainfield,
        grid_basis=cs_basis,
        settings=MockSettings(apply_m_imp_gauge=True),
        solution_space=cs_basis,
    )
    state_without_gauge = State(
        basis=sh_basis_dummy,
        mainfield=mainfield,
        grid_basis=cs_basis,
        settings=MockSettings(apply_m_imp_gauge=False),
        solution_space=cs_basis,
    )

    reduced_with = state_with_gauge.get_m_imp_reduced_system()
    reduced_without = state_without_gauge.get_m_imp_reduced_system()
    assert reduced_with.n_reduced < reduced_with.n_total
    assert reduced_without.n_reduced == reduced_without.n_total
    assert state_with_gauge.m_imp_feedback_system.problem.solution_size == reduced_with.n_reduced
    assert (
        state_without_gauge.m_imp_feedback_system.problem.solution_size
        == reduced_without.n_reduced
    )
    assert (
        state_with_gauge.m_imp_feedback_system.problem.num_data_terms
        == state_without_gauge.m_imp_feedback_system.problem.num_data_terms
    )
    np.testing.assert_allclose(
        np.asarray(state_without_gauge.constraints.m_imp_gauge_projector), np.eye(cs_basis.size)
    )
    assert not np.allclose(
        np.asarray(state_with_gauge.constraints.m_imp_gauge_projector), np.eye(cs_basis.size)
    )
