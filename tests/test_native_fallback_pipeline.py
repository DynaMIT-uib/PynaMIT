...
def _assert_close(name: str, native: Any, fallback: Any) -> None:
    ...
    index = np.unravel_index(flat_index, scaled_error.shape)
    fallback_norm = float(np.linalg.norm(fallback_array))
    difference_norm = float(np.linalg.norm(native_array - fallback_array))
    relative_l2 = difference_norm / fallback_norm if fallback_norm else float("inf")
    pytest.fail(
        f"{name}: native and fallback differ at {index}; "
        f"native={native_array[index]:.17e}, fallback={fallback_array[index]:.17e}, "
        f"abs_difference={difference[index]:.6e}, relative_l2={relative_l2:.6e}, "
        f"scaled_error={scaled_error[index]:.6e}"
    )
