    ...
    try:
        if current_vmin < current_vmax and not (np.isnan(current_vmin) or np.isnan(current_vmax)):
            ...
            if use_pcolormesh:
                ...
                im = ax.pcolormesh(lon_coords_2d, lat_coords_2d, data_to_plot_masked, cmap=cmap, vmin=current_vmin, vmax=current_vmax,
                                 transform=plot_transform, shading='auto', zorder=1)
            else:
                print(f"    Using contourf for '{title}' with transform={plot_transform}")
                if num_nans > 0: print(f"    NOTE: Data for '{title}' contains {num_nans} NaNs. Masking applied for contourf.")
                print(lat_coords_2d.shape, lon_coords_2d.shape, data_to_plot_masked.shape)
                im = ax.contourf(lon_coords_2d, lat_coords_2d, data_to_plot_masked, transform=plot_transform,
...
def plot_input_vs_interpolated(
    ...
    for row_idx, time_val_secs in enumerate(times_to_plot):
        ...
        for data_type_idx, data_type_str in enumerate(data_types_to_plot):
            ...
            if input_data_2d is not None and interpolated_data_2d is not None: # ... (vmin/vmax calc) ...
