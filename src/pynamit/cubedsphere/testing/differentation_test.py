...
print('Numerically calculated Br     matches Br     calculated with spherical harmonics: {}'.format(np.allclose(Br_num.flatten() - Br.flatten(), 0)))
...
print('Numerically calculated Btheta matches Btheta calculated with spherical harmonics: {}'.format(np.allclose(-u_north.flatten() - Btheta.squeeze()[:, 2:-2, 2:-2].flatten(), 0)))
print('Numerically calculated Bphi   matches Bphi   calculated with spherical harmonics: {}'.format(np.allclose( u_east.flatten()  - Bphi.squeeze()[:, 2:-2, 2:-2].flatten(), 0)))
...
print('Using sparse differentiation matrix: Numerically calculated Btheta matches Btheta calculated with spherical harmonics: {}'.format(np.allclose( (-u_north.reshape(shape) - Btheta.squeeze()).reshape(shape)[k_inner, i_inner, j_inner], 0)))
print('Using sparse differentiation matrix: Numerically calculated Bphi   matches Bphi   calculated with spherical harmonics: {}'.format(np.allclose( ( u_east.reshape(shape)  - Bphi.squeeze()  ).reshape(shape)[k_inner, i_inner, j_inner], 0)))
...
print('Numerically calculated eastward  components consistent with SH: {}'.format(np.all(np.isclose(u_east - Bphi, 0))))
print('Numerically calculated northward components consistent with SH: {}'.format(np.all(np.isclose(u_north + Btheta, 0))))
