...
from tests import conftest as test_configuration


def test_missing_jax_selects_only_numpy(monkeypatch):
    """The default suite must not request JAX when it is unavailable."""
    monkeypatch.setattr(test_configuration, "JAX_AVAILABLE", False)

    assert test_configuration._available_backends(None) == ["numpy"]


def test_missing_native_providers_selects_only_fallback(monkeypatch):
    """Do not request unavailable input providers by default."""
    monkeypatch.setattr(test_configuration, "native_inputs_available", lambda: False)

    assert test_configuration._available_sources(None) == ["fallback"]
