...
def test_evaluate_projected_scalar_input_on_model_grid(tmp_path):
    ...
    dynamics = pynamit.Dynamics(run_directory=tmp_path, Nmax=2, Mmax=1, Ncs=8, ignore_PFAC=True)
...
def test_evaluate_projected_input_corrects_explicit_transform_source(tmp_path):
    ...
    corrected = evaluate_projected_input(dynamics, "jr", 0.0, transform=wrong_source_transform)
