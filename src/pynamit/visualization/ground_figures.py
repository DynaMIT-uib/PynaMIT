...
class GroundFigureRenderer:
    ...
    def render_curve_map(self):
        ...
        curve_width_deg = 10.0 * float(self.spec.curve_time_scale)
        ...
        selection_extent = local_time_window_extent(
            lat_window=(self.spec.geo_lat_min, self.spec.geo_lat_max),
            local_time_window=(self.spec.local_time_min, self.spec.local_time_max),
            reference_time=target_times[0],
            central_longitude=central_longitude,
        )
        map_extent = selection_extent if self.spec.zoom_window else None
        ...
        show_low_latitude_guides = (
            self.spec.show_dip_equator_curve or self.spec.show_low_latitude_curve
        )
        ...
        if selection_extent is not None and not self.spec.zoom_window:
            self._draw_zoom_window_lines(
                axis, data_projection, selection_extent, central_longitude
            )

        if station_labels and self.spec.show_station_labels:
        ...
        title = (
            f"Ground {signal_label} Curve Map: {self.spec.ground_component}; "
    ...
    def render_timeseries(self):
        ...
        for axis, component in zip(axes, components, strict=True):
            ...
            axis.set_ylabel("nT/s" if self.spec.ground_quantity == "dbdt" else "nT")
    ...
    def _curve_sites_and_measurements(self, target_times, *, dbdt_cadence_seconds=None):
        ...
        lon, lat = build_even_global_sites(
            min_lat=-75.0,
            max_lat=75.0,
            lat_count=self.spec.ground_model_lat_count,
            equatorial_count=self.spec.ground_model_lt_count,
            min_sites_per_row=1,
            reference_time=target_times[0],
            visually_even=self.spec.ground_model_visual_even,
        )
    ...
    def _station_table(self):
        ...
        run_dir = Path(self.spec.run_directory).expanduser()
        repo_root = Path(__file__).resolve().parents[3]
        candidates = []
        if self.spec.data_directory:
            candidates.append(
                Path(self.spec.data_directory).expanduser() / "stations_full_list.csv"
            )
        candidates.extend(
            [
    ...
    def _ground_reference_line(self, target_times, display_scale):
        ...
        try:
            reference_time = pd.Timestamp(
                f"{target_times[0].date()} {self.spec.reference_time_of_day_utc}"
            )
        except ValueError:
            return None
    ...
    def _draw_reference_line_on_axis(self, axis, target_times):
        if not self.spec.show_reference_line:
            return
        try:
    ...
    def _draw_conductance_overlays(self, axis, data_projection, target_time):
        handles = []
        if not (
            self.spec.show_pedersen_conductance_overlay or self.spec.show_hall_conductance_overlay
        ):
            return handles
        ...
        overlay_specs = [
            (
                self.spec.show_pedersen_conductance_overlay,
                "sigmaP",
            ...
            (
                self.spec.show_hall_conductance_overlay,
                "sigmaH",
                "Hall conductance",
                "#1F78B4",
                (0, (4, 2)),
    ...
    def _draw_low_latitude_curves(self, axis, data_projection, central_longitude, reference_time):
        ...
        traces = []
        if self.spec.show_dip_equator_curve:
            traces.append((0.0, dip_equator_style))
        if self.spec.show_low_latitude_curve and boundary > 0.0:
