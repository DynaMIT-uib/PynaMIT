...
class TensorChain:
    ...
    def as_linear_operator(self) -> LinearOperator:
        ...
        def _matvec(x_flat):
            ...
            all_tensors = self.component_arrays + [x_tensor]
            res = np.einsum(
                self.einsum_string_matvec, *all_tensors, optimize=self.einsum_path_matvec
            )
        ...
        def _rmatvec(y_flat):
            ...
            conj_tensors = [arr.conj() for arr in self.component_arrays]
            all_adjoint_inputs = [grad_tensor] + conj_tensors
            grad_x = np.einsum(
                self.einsum_string_rmatvec, *all_adjoint_inputs, optimize=self.einsum_path_rmatvec
            )
    ...
    def component_arrays(self) -> list[np.ndarray]:
        ...
        return [np.asarray(t) for t in self.component_tensors]
    ...
    def rmatvec(self, y_flat: Any) -> Any:
        ...
        contract = self.contract_rmatvec
        if contract is not None:
    ...
    @functools.cached_property
    def einsum_path_matvec(self) -> list:
        """Return cached optimized einsum path for matvec."""
        dummy_input = np.empty(self.input_shape, dtype=self.dtype)
        return np.einsum_path(
            self.einsum_string_matvec, *self.component_arrays, dummy_input, optimize="greedy"
        )[0]

    @functools.cached_property
    def einsum_path_rmatvec(self) -> list:
