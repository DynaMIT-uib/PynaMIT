...
:mod:`pynamit.plotting.figure_builder` and the Panel app.
"""
...
class PynamEye:
    ...
    def __init__(
        ...
        """
        warnings.warn(
            "PynamEye is deprecated; use pynamit.plotting or pynamit.gui instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        optional_datasets = [
        ...
        self.results = SimulationResults.from_directory(
            simulation_directory,
            required_datasets=("conductance",),
            optional_datasets=optional_datasets,
            build_geometry=True,
        )
        if not {"dynamic", "equilibrium"} & self.results.datasets.keys():
            raise ValueError(
        ...
        self.gap_Br_response_matrix = (
            ...
            else self.results.geometry.boundary_jr_to_gap_Br_matrix
        )

        self.mlatlim = mlatlim
        self.settings = self.results.datasets["settings"]
        self.config = self.results.config
        self.RI = float(self.config.RI)
        self.main_field = self.results.main_field
        ...
        self.schema = self.results.schema
    ...
    def derive_E_from_B(self):
        ...
        if not self._e_from_b_cache_ready:
            # Reuse the saved simulation's exact numerical geometry.
