...
simulation.set_conductance(pedersen=pedersen, hall=hall, reg_lambda=0.001, grid=SphericalGrid(lat=conductance_lat, lon=conductance_lon))
...
simulation.set_neutral_wind(u_theta=u_theta, u_phi=u_phi, sqrt_weights=np.tile(np.sqrt(np.sin(np.deg2rad(90 - u_lat.flatten()))), (2, 1)), reg_lambda=0.001, grid=SphericalGrid(lat=u_lat, lon=u_lon))
...
simulation.evolve_to_time(0)  # Save simulation object with new m_ind
