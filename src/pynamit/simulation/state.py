...
class State(object):
    ...
    def set_conductance(self, etaP, etaH):
        ...
        if self.connect_hemispheres:
            self.E_coeffs_direct_to_E_coeffs_constraints = -np.tensordot(self.m_imp_to_E_coeffs.dot(self.constraints_least_squares.ATWA_plus_R_inv), np.tensordot(self.constraints_least_squares.ATW[2], self.E_coeffs_to_E_apex_perp_ll_diff, 2), 1) * self.ih_constraint_scaling
