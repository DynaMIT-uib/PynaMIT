...
class LeastSquaresSolver:
    ...
    def __init__(self, A, solution_ndim, weights=None, regularization_weights=None, regularization_matrices=None,
        ...
        self.sqrt_weights = [
            _ProcessedItem(np.sqrt(w.op), w.trailing_shape, w.leading_dim_count) if w is not None else None
    ...
    def _scaled_regularization_weights(self):
        ...
        num_features = self.A[0].op.shape[1]
        diag_data = np.sum([np.sum((self._densify_op(w) if w else 1.0) * (self._densify_op(a)**2), axis=0) for a, w in zip(self.A, self.weights)], axis=0)
    ...
    def _get_linear_operator(self, num_scenarios=1):
        """
        Creates the master LinearOperator for the least-squares problem, correctly
        handling both dense and matrix-free components for multi-scenario solves.
        """
        ...
        def _apply_op_to_block(item, x_block):
            ...
            if isinstance(item.op, LinearOperator):
                ...
                return res_block
            else: # Dense numpy array: use efficient block product
        ...
        # --- Block-based operations (operate on 2D arrays: [features, scenarios]) ---
        ...
        def matvec_final(x_flat):
            ...
            y_block = matvec_block(x_block)
            return y_block.flatten("F")
            
        def rmatvec_final(y_flat):
            y_block = y_flat.reshape(op_rows, num_scenarios, order="F")
            x_block = rmatvec_block(y_block)
            return x_block.flatten("F")
    ...
    def _solve_lsmr(self, proc_b, **kwargs):
        ...
        for i, b in enumerate(proc_b):
            if b is not None:
                num_scen_i = scenarios[i]
                sol_shape = self.A[0].trailing_shape + b.trailing_shape
                solutions.append(sol_block[:, offset : offset + num_scen_i].reshape(sol_shape))
                offset += num_scen_i
