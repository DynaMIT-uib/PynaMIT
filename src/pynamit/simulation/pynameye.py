...
class PynamEye(object):
    def __init__(self, filename_prefix, t = 0, Nlat = 60, Nlon = 100, NCS_plot = 10, mlatlim = 50, steady_state = False):
        ...
        for region in ['global', 'north', 'south']:
            self.G_B_pol_to_JS[region] = -self.evaluator[region].G_rxgrad * self.basis.V_external_to_delta_V / mu0
    ...
    def derive_E_from_B(self):
        ...
        if not self.B_parameters_calculated:
            ...
            self.G_B_pol_to_JS = -self.evaluator['num'].G_rxgrad * self.basis.V_external_to_delta_V / mu0
    ...
    def set_time(self, t, steady_state = False):
        ...
        self.m_u    = np.vstack(np.split(self.datasets['u'          ].SH_u    .sel(time = self.t, method='nearest').values, 2))
