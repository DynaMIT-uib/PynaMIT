...
def plot_scalar_map_on_ax(
    ...
    if use_pcolormesh:
        ...
        if isinstance(norm, mcolors.LogNorm):
            ...
            if abs(plot_vmax - plot_vmin) < 1e-12:
                levels = np.array([plot_vmin, plot_vmax])
            else:
...
def plot_input_vs_interpolated(
    ...
    for timestep in timesteps_to_plot:
        ...
        for data_type_str in data_types_to_plot:
            ...
            _, _, target_shape = (
                ...
                if details["grid_type"] == "magnetosphere"
                else (ionosphere_lon, ionosphere_lat, ionosphere_lat.shape)
            )

            input_data_2d = np.full(target_shape, np.nan)

            details["h5_key_primary"] = details["h5_key_primary"]

            if data_type_str == "Br":
                input_data_2d = h5file[details["h5_key_primary"]][timestep, :, :] * 1e-9
            elif data_type_str == "jr":
                input_data_2d = (
                    h5file[details["h5_key_primary"]][timestep, :, :] * 1e-6
                ) * ionosphere_br_2d
            elif data_type_str in ["SH", "SP"]:
                input_data_2d = h5file[details["h5_key_primary"]][timestep, :, :]
            ...
            if data_type_str == "u_mag":
                input_data_2d = np.sqrt(u_n**2 + u_e**2)
            elif data_type_str == "u_theta":
                input_data_2d = -u_n
            elif data_type_str == "u_phi":
                input_data_2d = u_e
            if input_data_2d is not None:
                all_data_for_scaling[data_type_str]["input"].append(input_data_2d.ravel())
            ...
            if timeseries_entry:
                ...
                current_lon_eval, current_lat_eval = (
                ...
                if data_type_str not in plot_evaluators:
                    plot_evaluators[data_type_str] = BasisEvaluator(
                        storage_basis, Grid(lat=current_lat_eval, lon=current_lon_eval)
                    )
                if pynamit_ts_key == "Br":
                    interpolated_data_2d = _calculate_interpolated_scalar_field(
                        timeseries_entry,
                        "Br",
                        storage_basis,
                        plot_evaluators[data_type_str],
                        target_shape,
                    )
                    ...
                    interpolated_data_2d = _calculate_interpolated_conductance(
                        timeseries_entry,
                        data_type_str,
                        storage_basis,
                        plot_evaluators[data_type_str],
                    ...
                    interpolated_data_2d = _calculate_interpolated_u_field(
                        timeseries_entry,
                        data_type_str,
                        storage_basis,
                        plot_evaluators[data_type_str],
    ...
    for data_type_str in data_types_to_plot:
        ...
        combined_flat = np.concatenate(
            (
                np.concatenate(cat_input_all) if cat_input_all else np.array([]),
                np.concatenate(cat_interp_all) if cat_interp_all else np.array([]),
            )
        )
    ...
    for dt_idx, data_type_str in enumerate(data_types_to_plot):
        ...
        ax_input_text_label = sfig_BL_cbars_and_labels.add_subplot(
            gs_bottom_left[row_idx_input, 1]
        )  # Changed from col 2 to col 1
        ax_input_text_label.text(
            0.5, 0.5, "Input", ha="center", va="center", rotation=90, fontsize=8
        )
        ax_input_text_label.axis("off")

        ax_fitted_text_label = sfig_BL_cbars_and_labels.add_subplot(
            gs_bottom_left[row_idx_fitted, 1]
        )  # Changed from col 2 to col 1
        ...
        for ts_idx, timestep in enumerate(timesteps_to_plot):
            ...
            if not (0 <= timestep < num_h5_steps):
                ax_input.text(
                    0.5, 0.5, "Data OOB", ha="center", va="center", transform=ax_input.transAxes
                )
                ax_input.set_xticks([])
                ax_input.set_yticks([])
                ax_fitted.text(
                    0.5, 0.5, "Data OOB", ha="center", va="center", transform=ax_fitted.transAxes
                )
                ax_fitted.set_xticks([])
                ax_fitted.set_yticks([])
                continue

            plot_title = ""  # Time labels are in sfig_TR_times

            current_lon, current_lat, target_shape = (
                (magnetosphere_lon, magnetosphere_lat, magnetosphere_lat.shape)
                if details["grid_type"] == "magnetosphere"
                else (ionosphere_lon, ionosphere_lat, ionosphere_lat.shape)
            )
            ...
            interpolated_data_2d = np.full(target_shape, np.nan)
            timeseries_entry = input_timeseries.get_entry(
                pynamit_timeseries_key_map[data_type_str], time_val, interpolation=False
            )
            if timeseries_entry:
                storage_basis = input_timeseries.storage_bases[
                    pynamit_timeseries_key_map[data_type_str]
                ]
                if pynamit_timeseries_key_map[data_type_str] == "Br":
                    interpolated_data_2d = _calculate_interpolated_scalar_field(
                        timeseries_entry,
                        "Br",
                        storage_basis,
                ...
                elif pynamit_timeseries_key_map[data_type_str] == "jr":
                    interpolated_data_2d = _calculate_interpolated_scalar_field(
                        timeseries_entry,
                        "jr",
                        storage_basis,
                ...
                elif pynamit_timeseries_key_map[data_type_str] == "conductance":
                    interpolated_data_2d = _calculate_interpolated_conductance(
                        timeseries_entry,
                        data_type_str,
                        storage_basis,
                ...
                elif pynamit_timeseries_key_map[data_type_str] == "u":
                    interpolated_data_2d = _calculate_interpolated_u_field(
                        timeseries_entry,
                        data_type_str,
                        storage_basis,
            ...
            if current_mappable_this_dt is None and not np.all(np.isnan(input_data_2d)):
            ...
            if current_mappable_this_dt is None and not np.all(np.isnan(interpolated_data_2d)):
        ...
        if current_mappable_this_dt:  # Check if there's a mappable for this data type
            ax_cbar = sfig_BL_cbars_and_labels.add_subplot(
                gs_dt_cbar_block[1]
            )  # Use the nested gs
