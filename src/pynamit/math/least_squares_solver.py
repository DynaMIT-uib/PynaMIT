...
class TensorChain:
    ...
    def to_dense(self) -> np.ndarray:
        ...
        return (dense_matrix * self.scaling_factor).reshape(math.prod(self.output_shape), math.prod(self.input_shape))
    ...
    def as_linear_operator(self) -> LinearOperator:
        ...
        if self._einsum_path_matvec is None:
            ...
            self._einsum_path_matvec = np.einsum_path(self.einsum_string_matvec, *self.component_tensors, dummy, optimize='greedy')[0]
        if self._einsum_path_rmatvec is None:
            dummy_grad = np.empty(self.output_shape, dtype=self.dtype)
            self._einsum_path_rmatvec = np.einsum_path(self.einsum_string_rmatvec, dummy_grad, *self.component_tensors, optimize='greedy')[0]
        ...
        def _rmatvec(y_flat):
            ...
            grad_x = np.einsum(self.einsum_string_rmatvec, *all_adjoint, optimize=self._einsum_path_rmatvec)
            return (grad_x.conj() * self.scaling_factor).flatten()

        return LinearOperator(shape=(flat_out, flat_in), matvec=_matvec, rmatvec=_rmatvec, dtype=self.dtype)
...
class LeastSquaresSolver:
    def __init__(
        ...
        self.regularization_matrices = [
            self._flatten(L, input_shape=self.solution_shape) if L is not None else None for L in reg_L_list
        ]
        ...
        if any(L is not None and isinstance(L.op, LinearOperator) for L in self.regularization_matrices):
    ...
    def update_matrices(self, A, sqrt_weights=None, data_shapes=None) -> None:
        ...
        if data_shapes is not None:
            ...
            raise ValueError("data_shapes must be provided when setting A for the first time or changing number of A operators.")
        ...
        for i, w_val in enumerate(sqrt_weights_list):
            ...
            is_diagonal = not isinstance(w_val, LinearOperator) and np.asarray(w_val).size == flat_data_dim
            if is_diagonal:
    ...
    def clear_cache(self, clear_preconditioner: bool = True) -> None:
        keys = [k for k in list(self._op_cache.keys()) if not k.startswith(("lsmr_components", "cg_components"))]
        for k in keys:
    ...
    def _flatten(self, op: Any, output_shape: tuple = None, input_shape: tuple = None) -> _ProcessedItem:
        if isinstance(op, TensorChain):
            if self.is_matrix_free:
                return _ProcessedItem(op=op.as_linear_operator(), output_shape=op.output_shape, input_shape=op.input_shape)
            return _ProcessedItem(op=op.to_dense(), output_shape=op.output_shape, input_shape=op.input_shape)
    ...
    def _calculate_and_cache_scaled_lambdas(self) -> None:
        ...
        for i, L_item in enumerate(self.regularization_matrices):
            ...
            for j in range(self.solution_size):
                ...
                col_j = (
                    L_op.matvec(e_j) if isinstance(L_op, LinearOperator) else self._densify_op(L_item)[:, j]
                )
    ...
    def _get_lsmr_components(self, num_scenarios):
        ...
        if self.preconditioner == "jacobi":
            ...
            def precond_matvec(y_flat): return base_op.matvec((y_flat.reshape(self.solution_size, num_scenarios) * sqrt_inv[:, None]).flatten())
            def precond_rmatvec(d_flat): return (base_op.rmatvec(d_flat.reshape(-1, num_scenarios).flatten()).reshape(self.solution_size, num_scenarios) * sqrt_inv[:, None]).flatten()
    ...
    def _get_cg_components(self, num_scenarios=1):
        ...
        cg_op = LinearOperator((self.solution_size * num_scenarios, self.solution_size * num_scenarios), matvec=normal_matvec, rmatvec=normal_matvec, dtype=base_op.dtype)
        M = None
        if self.preconditioner == "jacobi":
            self._setup_preconditioner_components()
            ...
            def precon_matvec(x_flat): return precon_block(x_flat.reshape(self.solution_size, num_scenarios)).flatten()
    ...
    def solve(self, b: Union[Any, List[Any]], **kwargs) -> np.ndarray:
        ...
        processed = [self._process_b_vector(b_val, self.data_shapes[i]) for i, b_val in enumerate(b_list)]
        ...
        for i in range(self.num_data_terms):
            ...
            if w_item is not None:
                ...
                if w_item.input_shape == (1,):
                    ...
                    grad_b_i = w_op.rmatmat(grad_d_i) if grad_d_i.shape[1] > 1 else w_op.rmatvec(grad_d_i[:, 0])[:, None]
                else:
