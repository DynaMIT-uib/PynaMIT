...
def test_ground_field_geographic_components_and_time_selection(tmp_path, main_field_kind):
    ...
    spaces = simulation.results.schema.output_field_spaces["dynamic"]
    for time, scale in ((0.0, 1.0), (10.0, 2.0)):
        ...
        simulation.results.output_series.add_entry("dynamic", values, time)
    ...
    for source in (simulation.results, results):
