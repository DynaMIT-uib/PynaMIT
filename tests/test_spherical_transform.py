...
def test_basis_evaluator_is_the_pynamit_compatibility_alias():
    ...
    coeffs[1] = 1.0
    np.testing.assert_allclose(
        evaluator.analyze_scalar(evaluator.synthesize_scalar(coeffs)), coeffs, atol=1e-10
    )
    ...
    np.testing.assert_allclose(evaluator.G_grad, np.stack([evaluator.G_th, evaluator.G_ph]))
    np.testing.assert_allclose(evaluator.G_rxgrad, np.stack([-evaluator.G_ph, evaluator.G_th]))
    np.testing.assert_allclose(evaluator.G_helmholtz[:, :, 0, :], -evaluator.G_grad)
    np.testing.assert_allclose(evaluator.G_helmholtz[:, :, 1, :], evaluator.G_rxgrad)
...
def test_spherical_transform_batches_direct_projection():
    ...
    scalar_actual = transform.project_scalar(
        scalar_values, input_grid=grid, projection_basis=basis
    )
    vector_actual = transform.project_helmholtz(
        vector_values, input_grid=grid, projection_basis=basis
    )
...
def test_spherical_transform_reuses_scalar_grid_remap(monkeypatch):
    ...
    projected_1 = transform.project_scalar(
        values, input_grid=input_grid, projection_basis=grid_remap_basis
    )
...
def test_spherical_transform_skips_matching_grid_remap(monkeypatch):
    ...
    projected = transform.project_scalar(
        values, input_grid=grid, projection_basis=grid_remap_basis
    )
...
def test_spherical_transform_requires_grid_remap_operator():
    ...
    with pytest.raises(TypeError, match="scalar_grid_remap_operator"):
        transform.project_scalar(values, input_grid=input_grid, projection_basis=target_basis)
...
def test_cs_non_native_scalar_analysis_solves_against_remap_operator():
    ...
    projected_rows = transform.project_scalar(
        value_rows, input_grid=target, projection_basis=basis
    )
...
def test_field_space_applies_cs_mean_free_after_spherical_projection():
    ...
    projected = transform.project_scalar(values, input_grid=grid, projection_basis=basis)
