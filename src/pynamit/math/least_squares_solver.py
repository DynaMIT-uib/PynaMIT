        ...
        if self.preconditioner == "jacobi":
            ...
            def precond_matvec(y_flat):
                return base_op.matvec(
                    (
                        y_flat.reshape(problem.solution_size, num_scenarios) * sqrt_inv[:, None]
                    ).flatten()
                )

            def precond_rmatvec(d_flat):
                return (
                    base_op.rmatvec(d_flat).reshape(problem.solution_size, num_scenarios)
                    * sqrt_inv[:, None]
                ).flatten()
            ...
            def precond_matvec(y_flat):
                return matvec_block(
                    p_matvec(y_flat.reshape(problem.solution_size, num_scenarios))
                ).flatten()
