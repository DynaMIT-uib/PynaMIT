...
class CSCoordinateSystem:
    """Stateless cubed-sphere coordinate and metric transforms."""
    ...
    def cube_to_cartesian(cls, xi, eta, r=1, block=0):
        ...
        mask = block == 0
        x[mask] = r[mask] / np.sqrt(delta[mask])
        y[mask] = r[mask] * np.tan(xi[mask]) / np.sqrt(delta[mask])
        z[mask] = r[mask] * np.tan(eta[mask]) / np.sqrt(delta[mask])

        mask = block == 1
        x[mask] = -r[mask] * np.tan(xi[mask]) / np.sqrt(delta[mask])
        y[mask] = r[mask] / np.sqrt(delta[mask])
        z[mask] = r[mask] * np.tan(eta[mask]) / np.sqrt(delta[mask])
        ...
        mask = block == 4
        x[mask] = -r[mask] * np.tan(eta[mask]) / np.sqrt(delta[mask])
        y[mask] = r[mask] * np.tan(xi[mask]) / np.sqrt(delta[mask])
        z[mask] = r[mask] / np.sqrt(delta[mask])
    ...
    @classmethod
    def cube_to_spherical(cls, xi, eta, block, r=1, deg=False):
        """Return spherical coordinates from CS coordinates."""
        xi, eta = np.float64(xi), np.float64(eta)
        xi, eta, r, block = np.broadcast_arrays(xi, eta, r, block)

        x, y, z = cls.cube_to_cartesian(xi, eta, r, block)
        phi = np.arctan2(y, x)
        theta = np.arccos(z / r)
    ...
    @classmethod
    def geo_to_cube(cls, lon, lat, block=None):
