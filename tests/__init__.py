...
def magnetic_potential_coordinate_array(simulation, output="dynamic"):
    """Return potential coordinates for historical regression values.

    The simulation now persists the physical ``induced_Br`` and
    ``boundary_jr`` fields. Historical numerical regression values were
    recorded in private poloidal- and toroidal-potential coordinates.
    Apply the exact inverse maps before comparing them.
    """
    dataset = simulation.results.output_series.datasets[output]
    ...
    boundary_jr_name = simulation.results.output_series.get_data_var_name(output, "boundary_jr")
    toroidal_potential = simulation.geometry.boundary_jr_to_toroidal_potential_operator.matvec(
