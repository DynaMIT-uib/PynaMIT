...
class State(object):
    ...
    def __init__(self, bases, pinv_rtols, reg_lambdas, mainfield, grid, settings, PFAC_matrix = None):
        ...
        if TRIPLE_PRODUCT and self.vector_conductance:
            ...
            self.etaP_m_ind_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, etaP_m_ind_to_E, 2), 2)

            etaH_m_ind_to_E = np.einsum('ijk,il->ijkl', self.m_ind_to_bH_JS, self.conductance_basis_evaluator.G, optimize = True)
            self.etaH_m_ind_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, etaH_m_ind_to_E, 2), 2)

            etaP_m_imp_to_E = np.einsum('ijk,il->ijkl', self.m_imp_to_bP_JS, self.conductance_basis_evaluator.G, optimize = True)
            self.etaP_m_imp_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, etaP_m_imp_to_E, 2), 2)

            etaH_m_imp_to_E = np.einsum('ijk,il->ijkl', self.m_imp_to_bH_JS, self.conductance_basis_evaluator.G, optimize = True)
            self.etaH_m_imp_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, etaH_m_imp_to_E, 2), 2)
        ...
        if self.vector_u:
            ...
            self.u_coeffs_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, u_coeffs_to_uxB, 2), 2)
        else:
            self.u_to_helmholtz_E = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.einsum('ijkl,lmk->ijkm', self.basis_evaluator.GTW_helmholtz, self.bu, optimize = True), 2)
    ...
    def set_conductance(self, etaP, etaH, vector_conductance = True):
        ...
        if TRIPLE_PRODUCT and self.vector_conductance:
            ...
            self.m_ind_to_helmholtz_E_direct = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, G_m_ind_to_E_direct, 2), 2)
            self.m_imp_to_helmholtz_E_direct = np.tensordot(self.basis_evaluator.GTWG_helmholtz_reg_inv, np.tensordot(self.basis_evaluator.GTW_helmholtz, G_m_imp_to_E_direct, 2), 2)
        ...
        if self.connect_hemispheres:
            A_imp_T_A_ind = np.tensordot(m_imp_apex_ll_diff_gram, -self.m_ind_to_helmholtz_E_direct, 2)
            ...
            if self.vector_u:
                ...
                A_imp = np.tensordot(self.helmholtz_to_apex_ll_diff, self.m_imp_to_helmholtz_E_direct, 2)
                self.cu_to_helmholtz_E  = np.tensordot(GTG_constraints_to_helmholtz_E, tensor_transpose(A_imp, 2), 1) * self.ih_constraint_scaling**2
