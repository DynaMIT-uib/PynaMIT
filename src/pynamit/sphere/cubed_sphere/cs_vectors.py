...
class CSVectorTransforms:
    """Stateless vector-frame transforms for the cubed-sphere grid."""
    ...
    def pc(xi, eta, r=1, block=0, inverse=False):
        ...
        delta = CSCoordinateSystem.delta(xi, et)
    ...
    def q_between_blocks(cls, xi, eta, block_i, block_j):
        ...
        psi_inv = cls.ps(xi_i, eta_i, r=1, block=block_i, inverse=True)
        _, theta, phi = CSCoordinateSystem.cube_to_spherical(
            xi_i, eta_i, r=1, block=block_i, deg=True
        )
        xi_j, eta_j, _ = CSCoordinateSystem.geo_to_cube(phi, 90 - theta, block=block_j)
        psj = cls.ps(xi_j, eta_j, r=1, block=block_j)
