...
for state_data in state_data_list:
    ...
    for i in range(25):
        axes.flatten()[i].plot(state_data.time.values, state_data['SH_m_imp'].values[:, i], label = '$B_r$')
...
for p, state_data in zip(periods, state_data_list):
    ...
    G_fourier = np.vstack((np.ones_like(t), np.cos(t / p * 2 * np.pi), np.sin(t / p * 2 * np.pi))).T
