...
# import matplotlib.ticker as mticker # No longer explicitly used
...
# SIMPLIFIED: plot_scalar_map_on_ax
def plot_scalar_map_on_ax(
    ...
    data_2d_arr,
    title="",
    cmap="viridis", # Should match the cmap norm was intended for
    norm=None,      # Expected to be a fully configured mcolors.Normalize or mcolors.LogNorm
):
    if norm is None:
...
def plot_input_vs_interpolated(
    ...
    # --- SIMPLIFIED: Pre-validate timesteps_to_plot and data_types_to_plot ---
    ...
    input_vars_pynamit = {
        "jr": {"jr": "scalar"}, "Br": {"Br": "scalar"},
        "conductance": {"etaP": "scalar", "etaH": "scalar"}, "u": {"u": "tangential"},
    }
    input_storage_bases = {
        "jr": sh_basis_zero_removed, "Br": sh_basis_zero_removed,
        "conductance": sh_basis, "u": sh_basis_zero_removed,
    }
    ...
    for timestep in timesteps_to_plot: # Timesteps are pre-validated
        ...
        for data_type_str in data_types_to_plot:
            ...
            if data_type_str == "Br":
                ...
                calculated_input_data_2d = (h5file[details["h5_key_primary"]][timestep, :, :] * 1e-6) * ionosphere_br_2d
                ...
                u_e_h5, u_n_h5 = (h5file[details["h5_key_primary"]][timestep, :, :], h5file[details["h5_key_secondary"]][timestep, :, :])
                if data_type_str == "u_mag": calculated_input_data_2d = np.sqrt(u_n_h5**2 + u_e_h5**2)
                elif data_type_str == "u_theta": calculated_input_data_2d = -u_n_h5
                elif data_type_str == "u_phi": calculated_input_data_2d = u_e_h5
            ...
            if timeseries_entry:
                ...
                if data_type_str not in plot_evaluators:
                    plot_evaluators[data_type_str] = BasisEvaluator(storage_basis, Grid(lat=current_lat_coords_pass1, lon=current_lon_coords_pass1))
                current_plot_evaluator = plot_evaluators[data_type_str]

                if pynamit_ts_key == "Br": calculated_interpolated_data_2d = _calculate_interpolated_scalar_field(timeseries_entry, "Br", storage_basis, current_plot_evaluator, target_shape_pass1)
                elif pynamit_ts_key == "jr": calculated_interpolated_data_2d = _calculate_interpolated_scalar_field(timeseries_entry, "jr", storage_basis, current_plot_evaluator, target_shape_pass1)
                elif pynamit_ts_key == "conductance": calculated_interpolated_data_2d = _calculate_interpolated_conductance(timeseries_entry, data_type_str, storage_basis, current_plot_evaluator, target_shape_pass1)
                elif pynamit_ts_key == "u": calculated_interpolated_data_2d = _calculate_interpolated_u_field(timeseries_entry, data_type_str, storage_basis, current_plot_evaluator, target_shape_pass1)
            ...
            cached_plot_data[cache_key] = {'input': calculated_input_data_2d, 'interpolated': calculated_interpolated_data_2d}
    ...
    for data_type_str in data_types_to_plot:
        ...
        if not combined_flat_list:
            h5file.close()
            raise ValueError(f"No data arrays found for '{data_type_str}' to calculate scales after Pass 1.")
        ...
        if temp_valid_data.size == 0:
            h5file.close()
            raise ValueError(f"No valid (non-NaN) data found for '{data_type_str}' to calculate scales.")

        if current_scale_type == "log":
            valid_data_for_percentile = temp_valid_data[temp_valid_data > 0]
            if valid_data_for_percentile.size == 0:
                h5file.close()
                raise ValueError(f"No positive valid data found for '{data_type_str}', as required for log scale.")
        else:
        ...
        if vmin == vmax:
            if not (current_scale_type == "log"): 
                 print(f"Warning: vmin ({vmin:.3e}) and vmax ({vmax:.3e}) are identical for '{data_type_str}'. Plot may show a single color.")

        norm_for_plot = None
        if current_scale_type == "log" and cmap_global != "bwr": 
            if vmin <= 0: 
                h5file.close()
                raise ValueError(f"Log scale for '{data_type_str}' requires vmin > 0, but got vmin={vmin:.3e}.")
            # mcolors.LogNorm itself will raise ValueError if vmin >= vmax.
            # We can catch it here for a more specific message if desired, or let LogNorm raise it.
            # For now, rely on LogNorm's own check for vmin < vmax.
            try:
                norm_for_plot = mcolors.LogNorm(vmin=vmin, vmax=vmax, clip=True)
            except ValueError as e:
                h5file.close()
                raise ValueError(f"Error creating LogNorm for '{data_type_str}' with vmin={vmin:.3e}, vmax={vmax:.3e}: {e}")

        else: 
            norm_for_plot = mcolors.Normalize(vmin=vmin, vmax=vmax, clip=True)

        global_plot_scales[data_type_str] = {
            "vmin": vmin, "vmax": vmax, "cmap": cmap_global,
            "norm": norm_for_plot, "scale_type": current_scale_type
        }
        print(f"  Global scale for '{data_type_str}' ({current_scale_type}): vmin={vmin:.3e}, vmax={vmax:.3e}")
    ...
    # Using original figure size estimation logic
    ...
    fig_height = min(
        max(4 + time_row_h_frac * fig_height_est, fig_height_est), # Ensure min height for time labels
        num_plot_rows * (base_plot_h + 0.15) + 1.0, # Cap overall height
    )
    ...
    sfigs_grid = fig.subfigures(2, 2, height_ratios=[time_row_h_frac, plots_row_h_frac], width_ratios=[cbars_labels_col_w_frac, plots_col_w_frac], hspace=0.01, wspace=0.01)
    sfigs_grid[0, 0].patch.set_alpha(0.0); [ax.remove() for ax in sfigs_grid[0, 0].get_axes()] 
    ...
    if num_plot_cols > 0: 
        ...
        for ts_idx, timestep in enumerate(timesteps_to_plot):
            time_label_axes[ts_idx].text(0.5, 0.5, f"{timestep * input_dt}s", ha="center", va="center", fontsize=9)
            time_label_axes[ts_idx].axis("off")

    map_axes_flat = sfigs_grid[1, 1].subplots(num_plot_rows, num_plot_cols, sharex=True, sharey=True, subplot_kw={"projection": ccrs.PlateCarree(central_longitude=noon_longitude)})
    ...
    gs_bottom_left = gridspec.GridSpec(num_plot_rows, 2, figure=sfigs_grid[1, 0], width_ratios=[0.8, 0.2], wspace=0.05, hspace=0.05)
    ...
    for dt_idx, data_type_str in enumerate(data_types_to_plot):
        ...
        gs_dt_cbar_block = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs_bottom_left[row_idx_input:row_idx_input + 2, 0], width_ratios=[0.4, 0.6], wspace=0.1)
        ax_dt_label = sfigs_grid[1, 0].add_subplot(gs_dt_cbar_block[0])
        ax_dt_label.text(0.5, 0.5, details["label"], ha="center", va="center", rotation=90, fontsize=9); ax_dt_label.axis("off")
        
        ax_input_text_label = sfigs_grid[1, 0].add_subplot(gs_bottom_left[row_idx_input, 1])
        ax_input_text_label.text(0.5, 0.5, "Input", ha="center", va="center", rotation=90, fontsize=8); ax_input_text_label.axis("off")
        ax_fitted_text_label = sfigs_grid[1, 0].add_subplot(gs_bottom_left[row_idx_fitted, 1])
        ax_fitted_text_label.text(0.5, 0.5, "Fitted", ha="center", va="center", rotation=90, fontsize=8); ax_fitted_text_label.axis("off")
        ...
        for ts_idx, timestep in enumerate(timesteps_to_plot):
            ...
            current_lon_plot, current_lat_plot = (magnetosphere_lon, magnetosphere_lat) if is_magnetosphere_grid_plot else (ionosphere_lon, ionosphere_lat)
            ...
            try:
                ...
                h5file.close() 
                raise RuntimeError(f"Critical: Data for {cache_key} not found in cache. This indicates an internal logic error.")

            im_input = plot_scalar_map_on_ax(ax_input, current_lon_plot, current_lat_plot, retrieved_input_data,
                                             title="", cmap=cmap_use, norm=norm_use)
            if current_mappable_this_dt is None and not np.all(np.isnan(retrieved_input_data)):
                current_mappable_this_dt = im_input

            im_fitted = plot_scalar_map_on_ax(ax_fitted, current_lon_plot, current_lat_plot, retrieved_interpolated_data,
                                              title="", cmap=cmap_use, norm=norm_use)
        ...
        if current_mappable_this_dt:
            ...
            ax_cbar.text(0.5, 0.5, "No Valid Data\nfor Colorbar", ha="center", va="center", fontsize=7, wrap=True)
            ax_cbar.axis("off")
