...
arrays merely to persist them. SH Helmholtz and gap-response analysis likewise
...
object identity. Resistance-dependent closure matrices, equilibrium inverses,
...
and basis constructions, together with the existing gap-response sidecar, are the
...
only to the evolving `induced_Br` field.
...
SM-to-GEO preparation transform and plot centering, not by rotating `induced_Br`
or the background field.
