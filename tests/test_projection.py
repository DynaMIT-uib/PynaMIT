...
def test_projection():
    ...
    geo2cube2spherical_works = np.allclose(90 - np.rad2deg(theta) - lat, 0) & np.allclose(np.rad2deg(phi) - lon, 0)
    print('Conversion from (lon, lat, block) to (xi, eta) and back works: {}'.format(geo2cube2spherical_works))

    assert geo2cube2spherical_works

    x, y, z = p.cube2cartesian(xi, eta, r = rr, block = block)
    geo2cube2cartesian_works = np.allclose(x - xx, 0) & np.allclose(y - yy, 0) & np.allclose(z - zz, 0)
    print('Conversion from (x, y, z, block ) to (xi, eta) and back works: {}'.format(geo2cube2cartesian_works))
    ...
    print('Converting vector components between cubed sphere and Cartesian give consistent results: {}'.format(cubed2cartesian_works))
    cubed2spherical_works = np.allclose(A_ - A, 0)
    print('Converting vector components between cubed sphere and spherical give consistent results: {}'.format(cubed2spherical_works))
    norm_consistent = np.allclose(np.linalg.norm(Asph_normed, axis = 0) - np.linalg.norm(Axyz, axis = 0), 0)
    print('Cartesian and normalizd spherical vectors have the same norm: {}'.format(norm_consistent))
    ...
    for i in range(6):
        ...
        Pc = p.get_Pc(xi, eta, r = np.array(1.), block = np.array(i), inverse = True)
    ...
    for block in range(6):
        r_hd, theta_hd, phi_hd = p.cube2spherical(xihd, etahd, r = np.array(1.), block = np.array(block))
        la = 90 - np.rad2deg(theta_hd)
        lo = np.rad2deg(phi_hd)

        r, theta, phi = p.cube2spherical(xi, eta, r = np.array(1.), block = np.array(block))
        Ps = p.get_Ps(xi, eta, r = np.array(1.), block = np.array(block))
        Ps = p.get_Ps(xi, eta, r = np.array(1.), block = np.array(block))
